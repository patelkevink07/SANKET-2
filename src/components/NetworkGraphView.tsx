...
export const NetworkGraphView: React.FC<NetworkGraphViewProps> = ({ onSelectNode }) => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Draw subtle grid
      ctx.strokeStyle = isDark ? '#273248' : '#e5e7eb';
      ctx.lineWidth = 1;
      const step = 40;
      ...
      edges.forEach((edge) => {
        ...
        if (edge.type === 'retweet') {
          ctx.strokeStyle = isDark ? '#5b8fd6' : '#003366';
          ctx.setLineDash([]);
        } else if (edge.type === 'reply') {
          ctx.strokeStyle = '#fe6500';
          ctx.setLineDash([4, 4]);
        } else {
          ctx.strokeStyle = isDark ? '#606e8c' : '#737780';
          ctx.setLineDash([2, 2]);
        }
      ...
      activeNodes.forEach((node) => {
        ...
        if (isSelected || isHovered) {
          ...
          ctx.fillStyle = isSelected ? 'rgba(254, 101, 0, 0.3)' : (isDark ? 'rgba(91, 143, 214, 0.25)' : 'rgba(0, 51, 102, 0.2)');
          ctx.fill();
        }
        ...
        if (node.role === 'Bot') {
          ...
          ctx.fillStyle = isDark ? '#4b75a8' : '#001e40';
        } else if (node.role === 'KOL') {
          ctx.fillStyle = '#fe6500';
        } else {
          ctx.fillStyle = isDark ? '#326bb5' : '#003366';
        }
        ctx.fill();
        ...
        // Node Label
        ctx.fillStyle = isDark ? '#e3e9f5' : '#1a1c1c';
        ctx.font = 'bold 11px "Noto Sans", sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(node.label, node.x, node.y + r + 14);

        // Subtext (Handle)
        ctx.fillStyle = isDark ? '#8d9bb5' : '#737780';
  ...
  return (
    <div className="bg-surface border border-border-main p-4 flex flex-col gap-4 shadow-sm">
      {/* Top Toolbar */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border-subtle pb-3">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs">
          <span className="flex items-center gap-1">
            <span className="w-3 h-3 rounded-full bg-brand-navy-dark"></span>
            <span className="text-text-primary font-medium">Official</span>
          </span>
          <span className="flex items-center gap-1">
            <span className="w-3 h-3 rounded-full bg-accent-saffron"></span>
            <span className="text-text-primary font-medium">KOL (High Centrality)</span>
          </span>
          <span className="flex items-center gap-1">
            <span className="w-3 h-3 rounded-full bg-brand-navy-mid"></span>
            <span className="text-text-primary font-medium">Amplifier</span>
          </span>
          <span className="flex items-center gap-1">
            <span className="w-3 h-3 rounded-full bg-status-danger"></span>
            <span className="text-text-primary font-medium">Suspected Bot</span>
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 relative bg-surface-muted border border-border-main rounded overflow-hidden">
          <canvas
            ref={canvasRef}
            width={750}
            height={480}
          ...
          <div className="absolute bottom-3 left-3 right-3 bg-surface/95 backdrop-blur-xs p-3 border border-border-main rounded shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <button
                onClick={() => setIsPlaying(!isPlaying)}
                className="p-1.5 bg-brand-navy-mid hover:bg-brand-navy text-white rounded transition-colors flex items-center justify-center cursor-pointer"
              ...
              <span className="text-xs font-bold text-text-dark-navy">Propagation Step {temporalStep}/5</span>
            </div>

            <div className="flex-grow max-w-xs w-full flex items-center gap-2">
              <span className="text-[10px] text-text-muted font-mono">T-0h</span>
              <input
                type="range"
                min={1}
                max={5}
                ...
                className="w-full h-1.5 bg-surface-muted rounded-lg appearance-none cursor-pointer accent-brand-navy-mid"
              />
              <span className="text-[10px] text-text-muted font-mono">T-24h</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              <label className="flex items-center gap-1.5 text-xs text-text-primary font-medium cursor-pointer">
                <input
                  ...
                  className="rounded border-border-strong text-status-danger focus:ring-status-danger"
                />
        ...
        <div className="bg-canvas border border-border-main p-4 flex flex-col justify-between rounded">
          {selectedNode ? (
            <div className="space-y-4">
              <div className="flex items-start justify-between border-b border-border-main pb-3">
                <div>
                  ...
                  <h4 className="font-serif-headline text-lg font-bold text-text-dark-navy mt-1">
                    {selectedNode.label}
                  </h4>
                  <p className="text-xs text-text-muted font-mono">{selectedNode.username}</p>
                </div>
                <div className="text-right">
                  <span className="text-xs text-text-muted">Platform</span>
                  <p className="text-xs font-bold text-text-navy uppercase">{selectedNode.platform}</p>
                </div>
              </div>

              {/* Node Metrics */}
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="bg-surface p-2.5 border border-border-subtle rounded">
                  <span className="text-text-muted block text-[10px] uppercase">Betweenness Score</span>
                  <span className="text-sm font-bold text-text-navy">
                    {(selectedNode.betweennessCentrality * 100).toFixed(1)}%
                  </span>
                </div>
                <div className="bg-surface p-2.5 border border-border-subtle rounded">
                  <span className="text-text-muted block text-[10px] uppercase">Eigenvector Rank</span>
                  <span className="text-sm font-bold text-text-navy">
                    {(selectedNode.eigenvectorCentrality * 100).toFixed(1)}%
                  </span>
                </div>
                <div className="bg-surface p-2.5 border border-border-subtle rounded">
                  <span className="text-text-muted block text-[10px] uppercase">Followers Reached</span>
                ...
                <div className={`p-2.5 border rounded ${
                  selectedNode.botProbability > 0.5
                    ? 'bg-status-danger-light border-status-danger text-status-danger-text'
              ...
              {/* Dominant Polarity */}
              <div className="bg-surface p-3 border border-border-subtle rounded">
                <div className="flex justify-between items-center text-xs mb-1">
                  <span className="font-semibold text-text-secondary">Dominant Emotion Stance</span>
                  <span className="font-bold uppercase text-text-navy">{selectedNode.dominantSentiment}</span>
                </div>
              ...
              <div className="space-y-2 pt-2">
                <button
                  ...
                  className="w-full py-2 bg-brand-navy-mid hover:bg-brand-navy text-white font-bold text-xs uppercase tracking-wider rounded transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                >
                ...
                {selectedNode.botProbability > 0.5 && (
                  <button
                    ...
                    className="w-full py-2 bg-status-danger hover:bg-status-danger-hover text-white font-bold text-xs uppercase tracking-wider rounded transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                  >
