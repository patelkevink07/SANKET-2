...
export const ReportsView: React.FC<ReportsViewProps> = ({ user, onNavigate }) => {
  ...
  const [classification] = useState('DEMO ASSESSMENT // PROTOTYPE');
  ...
  const handleGenerate = () => {
    ...
    setTimeout(() => {
      ...
      const compilerName = user ? user.name : 'Demo Analyst';
      const compilerRole = user ? user.role : 'Demo Analyst';

      let reportContent = '';

      if (reportType === 'demographic_macro') {
      ...
      <div className="max-w-[1000px] mx-auto px-4 md:px-8">
        ...
        <div className="bg-surface border border-main p-6 md:p-8 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6 text-xs">
            ...
            <div>
              <label className="block font-bold text-primary mb-1 uppercase">Time Period</label>
              <select
                value={timeWindow}
